	...
	try {
		...
		citApp.retrieveSystemProperties(citApp, function () {
			try {
				...
				citApp.checkForLogLevel(citApp);
				citApp.loadMainView();
			} catch (e) {
...
CITApp.prototype.loadBootstrap = function () {
	try {
		...
		alert("Could not load bootstrap configuration because: "+e.stack);
	}
};
